...
from PyQt5.QtWidgets import QApplication, QMainWindow, QLabel, QLineEdit, QPushButton, QVBoxLayout, QWidget, \
     QInputDialog, QMessageBox, QRadioButton, QComboBox, QHBoxLayout
...
class MainWindow(QMainWindow):
    ...
    def on_mouse_press(self, event):
        ...
        if self.radio_edit.isChecked() and event.button == MouseButton.LEFT:
            self.grid_edit(event.xdata, event.ydata)
        if self.radio_krep.isChecked() and self.rectangles and event.button == MouseButton.LEFT:
            self.add_krep(event.xdata)
        if event.button == MouseButton.LEFT:
            if self.info_layers:
                self.add_line(event.xdata, event.ydata)
            else:
                msg = QMessageBox()
                msg.setWindowTitle("Ошибка")
                msg.setText("Возникли проблемы со слоями!")
